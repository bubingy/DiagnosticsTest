...
def init_test(test_conf: LTTngTestConfiguration) -> None:
    ...
    for run_conf in test_conf.run_conf_list:
        if 'win' in SysInfo.rid:
            env_script_path = os.path.join(test_conf.test_bed, f'env_activation_net{run_conf.dotnet_sdk_version}.ps1')
            lines = [
                f'$Env:DOTNET_ROOT={run_conf.dotnet_root}\n',
                f'$Env:Path+=;{run_conf.dotnet_root}\n'
            ]
        else:
            env_script_path = os.path.join(test_conf.test_bed, f'env_activation_net{run_conf.dotnet_sdk_version}.sh')
            lines = [
